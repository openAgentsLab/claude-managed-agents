...
// Tenant-level secrets (userID=="") use "tenant:<tenantID>" so their encryption
// keys are isolated from any user key.
...
func (r *secretRepo) List(_ context.Context, tenantID, userID string) ([]store.SecretMeta, error) {
	rows, err := r.db.Query(
		`SELECT name, description, updated_at FROM vault_secrets WHERE tenant_id = ? AND user_id = ? ORDER BY name ASC`,
		tenantID, userID,
	)
	if err != nil {
...
func (r *secretRepo) Delete(_ context.Context, tenantID, userID, name string) error {
	_, err := r.db.Exec(
		`DELETE FROM vault_secrets WHERE tenant_id = ? AND user_id = ? AND name = ?`,
...
// fetchSecret queries vault_secrets for the given (tenantID, userID, name) and
// returns the crypto identity, ciphertext, and nonce. Returns sql.ErrNoRows when
// the secret does not exist.
func (r *secretRepo) fetchSecret(tenantID, userID, name string) (cid string, ciphertext, nonce []byte, err error) {
	err = r.db.QueryRow(
		`SELECT encrypted_value, nonce FROM vault_secrets WHERE tenant_id = ? AND user_id = ? AND name = ?`,
		tenantID, userID, name,
...
func wrapSecretErr(op string, err error) error {
	...
	return fmt.Errorf("store sqlite secrets %s: %w", op, err)
}
