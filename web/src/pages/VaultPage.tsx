...
import { listVaults, setVault, deleteVault, type VaultItem } from '@/api/vault'
...
export default function VaultPage() {
  ...
  async function handleSave(name: string, value: string, description: string) {
    ...
    setError('')
    try {
      await setVault(name, value, description)
    ...
    <AppLayout>
      <div className="p-6 max-w-2xl mx-auto space-y-5">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              Encrypted secrets for MCP server credentials. Reference with <code className="font-mono text-xs">vault:NAME</code>.
            </p>
          </div>
          {!showForm && (
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-sm text-muted-foreground">No secrets stored</div>
        ) : (
